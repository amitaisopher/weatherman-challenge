...
export async function getCurrentWeather({ useDummyData = false, delay = 0, apiKey = '', units = 'metric', location = 'haifa' }) {
    const weatherHistoryEndpointurl = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${apiKey}&units=${units}`
    if (useDummyData) {
...
export async function getWetaherHistory({ useDummyData = false, delay = 0, apiKey = '', units = 'metric', location = 'haifa' }) {
...
export async function getIrradiationForecast(useDummyData = false, delay = 0, apiKey = '',) {
